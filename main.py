...
class EyeTracker(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    #                                    (self.l_video.width(), self.l_video.height()))  # 把读到的帧的大小重新设置为 640x480
    ...
    #         self.l_video.setPixmap(QtGui.QPixmap.fromImage(self.srcFrame))  # 往显示视频的Label里 显示QImage
    ...
    def monitor_eyes(self):
        # 定义警报标志
        global close_counter, open_counter
            ...
            if not ret:
                ...
                for rect in rects:
                    ...
                    frame = QtGui.QImage(frame.data, frame.shape[1], frame.shape[0],
                                                 int(frame.shape[1]) * 3,
                                                 QtGui.QImage.Format_RGB888)  # 把读取到的视频数据变成QImage形式
                    self.watch.setPixmap(QtGui.QPixmap.fromImage(frame))  # 往显示视频的Label里 显示QImage
